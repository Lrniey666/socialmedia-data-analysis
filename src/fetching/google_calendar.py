"""Fetch public holiday events from Google Calendar holiday calendars.

Copy config.example/google_calendar_credentials.json to
secrets/google_calendar_credentials.json (OAuth desktop client).
"""
...
client_secrets_file = str(ROOT / "secrets" / "google_calendar_credentials.json")
token_file = str(ROOT / "secrets" / "google_calendar_token.json")
...
def main():
    ...
    holidays = get_holiday(creds, UK_CALENDAR_ID, (2022, 1, 1), (2023, 1, 1), max_results=365)
    data = pd.DataFrame(holidays)
    print(data)
    out = ROOT / "data" / "Google Calendar" / "UK_holidays.csv"
    out.parent.mkdir(parents=True, exist_ok=True)
    data.to_csv(out, index=False)
    print(f"saved to {out}")

def get_holiday(creds, calendar_id, start_date= None, end_date= None, max_results= 10):
    try:
